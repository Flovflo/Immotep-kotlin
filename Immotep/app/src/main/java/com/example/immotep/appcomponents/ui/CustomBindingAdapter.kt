package com.example.immotep.appcomponents.ui
...
fun setVisibility(view: View, isVisible: Boolean, isInvisible: Boolean) {
    view.visibility = if (isVisible) View.VISIBLE else if (isInvisible) View.INVISIBLE else View.GONE
}

@BindingAdapter(value = ["imageUrl", "placeholder", "placeholderError", "cornerRadius", "circular"], requireAll = false)
fun loadImageFromNetwork(
    imageView: ImageView, url: String?, placeholder: Drawable?,
    placeholderError: Drawable?, cornerRadius: Float, circular: Boolean
) {
    var imageURL = url
    if (imageURL != null && imageURL.isNotEmpty() && !imageURL.equals(" ", ignoreCase = true)) {
        ...
        val options = RequestOptions()
            ...
            .apply { if (circular) circleCrop() else if (cornerRadius > 0) transform(CenterCrop(), RoundedCorners(cornerRadius.toInt())) }
...
@BindingAdapter(value = ["imageRes", "cornerRadius", "circular"], requireAll = false)
...
fun addSpaceBetweenRecyclerItem(
    ...
    recyclerView.addItemDecoration(RecyclerView.ItemDecoration(itemSpace.toInt(), includeEdge))
}
